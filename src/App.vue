      ...
      <div class="container">
        <div class="row p-5 text-center">
          <div class="form-group" v-if="showForm ">
            ...
            <select v-model="color" id="color" class="form-control" @change='handleChange($event)'>
               <option :value="mycolor.name" v-for="mycolor in colors" :key="mycolor.name">
                  {{mycolor.name }}
               </option>
            </select>
         </div>
...
export default {
  ...
  methods:{
    ...
    handleChange(event){
          this.$refs.couleur.setAttribute("style","background-color:"+event.target.value)
           },
           //show or hide form
           displayForm() {
